...
class FinanceView(QWidget):
    ...
    def load_data(self):
        ...
        with db_manager.get_connection() as conn:
            ...
            self.card_income.update_value(lang_manager.localize_digits(f"{total_income:,.0f} AFN"))
            self.card_expense.update_value(lang_manager.localize_digits(f"{total_expense:,.0f} AFN"))
            self.card_profit.update_value(lang_manager.localize_digits(f"{net_profit:,.0f} AFN"))
            
            self.load_expense_table(cursor, expense_filter)
            self.load_sales_summary_table(cursor, sales_filter)
    ...
    def load_expense_table(self, cursor, filter_sql):
        ...
        for i, row in enumerate(rows):
            ...
            self.expense_table.setItem(i, 0, QTableWidgetItem(str(row[0])))
            self.expense_table.setItem(i, 1, QTableWidgetItem(row[1]))
            self.expense_table.setItem(i, 2, QTableWidgetItem(str(row[2])))
            self.expense_table.setItem(i, 3, QTableWidgetItem(row[3] or "Unknown"))
            self.expense_table.setItem(i, 4, QTableWidgetItem(row[4] or ""))
